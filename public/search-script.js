...
class search_script {
  ...
  getNewLinks(callback, callback_sender) {
    request(this.url, (error, responce, html) => {
      if (error) {
        ...
        listings.each((i, single_listing) => {
          ...
          // ----------
          // get image
          const $image = $single_listing.find(
            ".s-item__image-wrapper .s-item__image-img"
          );
          ...
          const imageSRC =
            $image.attr("src") !== "https://ir.ebaystatic.com/cr/v/c1/s_1x2.gif"
              ? $image.attr("src")
              : $image.attr("data-src");
          // ----------
          // get title
        ...
        callback({ error: false, not_seen: not_seen }, callback_sender);
      }
    });
  }
}
